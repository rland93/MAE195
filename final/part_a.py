...
if draw["3"]:
    ...
    # Error analysis plot
    fig3 = plt.figure(num="RMS Error vs h", figsize=(7, 4))
    ax3 = fig3.add_subplot(111)
    all_figures.append(fig3)
    ax3.plot(hs, rmses, "o-", label="RMS Error")
    ax3.plot(hs, Oh, linestyle="--", label=r"$\mathcal{O}(h)$")
    ax3.plot(hs, Oh2, linestyle="--", label=r"$\mathcal{O}(h^2)$")
    ax3.set_xlabel(r"$h$")
    ax3.set_ylabel("RMSE")
    ax3.set_title(r"Error vs Step Size, SS Heat Transfer, Constant $\theta_{fluid}$")
    ax3.set_xscale("log")
    ax3.set_yscale("log")
    ax3.legend()
    plt.show()
...
if draw["4"]:
    ...
    fig4, ax4 = plt.subplots(
        ...
        tight_layout=True,
        figsize=(9, 4.5),
        num="Temperature Distributions for Different Parameters",
    )
    markers = ("o", "s", "^", "v")
    ...
    for case, (i, j), m in zip(cases, np.ndindex(ax4.shape), markers):
        ...
        ax4[i, j].scatter(xi, theta, marker="^", color="k", label="Numerical Solution")
...
if draw["5"]:
    ...
    for problem in problems_random:
        ...
        for conv, label in zip((romberg4, trap, trap2), ("Romberg4", "Trap", "Trap2")):
            cond = left - right
    ...
    ax5.scatter(betas, errs_romb, label=r"Romberg (order 4)", marker="+")
    ax5.scatter(betas, errs_romb6, label=r"Romberg (order 6)", marker=".")
    ax5.scatter(betas, errs_trap12, label=r"Trapezoid ($2 h$)", marker="x")
    ax5.scatter(betas, errs_trap, label=r"Trapezoid ($h$)", marker="3")
    ax5.scatter(betas, errs_trap2, label=r"Trapezoid ($0.5 h$)", marker="4")
